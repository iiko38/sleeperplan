...
**Approval binding.** The plan carries a `physical_design_hash` — a SHA-256 over the physical artefact only (generator version, profile, beds, orientation, stock lengths/trims, screw lengths/diameters/pilot spec, geometric rules). Prices, pack sizes, availability, commercial rules (spares allowance, price-age limits), provenance text, the review block and the as-of date are excluded, so a price-only revision does not invalidate a physical approval, while any change to dimensions, hardware or machining does. Hardware selection is itself price-independent (`length, diameter, id`): repricing can never swap the installed screw. `--release` requires `review.approved_physical_design_hash` to equal the current hash: a mismatch blocks release (`stale_approval`), and a missing hash blocks draft, check and release (`approval_hash_required`); plans carry the unified `release_ready` field. Workflow: run a draft plan, copy the printed physical design hash into the job's review block once the physical review is genuinely done, then release. `examples/reviewed-example.json` and `catalogues/wickes-reviewed-2026-09-06.json` are DEMONSTRATION fixtures for this gate — their reviewer/pilot text is not real evidence.

**Evidence kinds.** Confirmed pilot modes require `pilot_evidence_kind`: `manufacturer_instruction`, `recorded_trial`, or `fixture`. Fixture evidence is test/demonstration data and blocks issuance unconditionally (`fixture_evidence`), whatever the evidence text claims. Software cannot verify the truth of a human trial; it can refuse known test data.

**Operations schedule.** Every export compiles `operations.json` (`sleeperplan.operations.v1`): an ordered, typed list of workshop operations compiled from the same geometry — receive/label/cut/treat per stock board, place/clamp per course, then mark-STOP or drill + drive per fixing, then supports/final checks. Dependencies are explicit; a drive operation always follows its course clamp and its own preparation. Unknown physical values (pilot spec, head seat) surface as mark-out-only/STOP operations, never invented machining instructions. The web assembly player renders this schedule deterministically: displayed state is a pure function of plan + operation index + progress, and screw insertion uses `head(u) = E - d*(L+g)*(1-u)` so the final frame equals the approved modelled position exactly.
